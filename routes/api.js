...
/** Generate a new Pyro App
 * @external Endpoint /generate
		...
		pyrofb.child('instances').child(newAppName).once('value', function(appSnap){
			...
			if(!appSnap.val()){
				...
				generatePyroApp(req.body.uid, req.body.name).then(function(pyroAppData){
					...
					//[TODO] Handle App Firebase already existing
...
router.post('/app/copy', function(req, res){
	console.log('new request received:', req.body);
	if(req.body.hasOwnProperty('name') && req.body.hasOwnProperty('uid')){
		console.log('[/generate] request is the correct shape');
		var newAppName = req.body.name;
		pyrofb.child('instances').child(newAppName).once('value', function(appSnap){
				...
				if(!appSnap.val()){
					...
					// Log into Server Firebase account
					// generateFirebase
					uploadToBucket("pyro-"+newAppName, "fs/seed", newAppName).then(function(pyroAppData){
						console.log('[/app/copy App copied successfully]:', pyroAppData);
						pyroAppData.status = 200;
						respond(pyroAppData, res);
					}, function(err){
						respond(err, res);
...
/** Upload file contents to live version of application
 * @external Endpoint /app/upload
 * @params {string} Name Name of list to retreive
 * @params {string} Uid - Unique id of user to generate for
 * @params {string} filePath - Path of file being uploaded
 * @params {string} content - String Contents of file to be uploaded
 */
router.post('/app/upload', function(req, res){
	console.log('new request received:', req.body);
	if(req.body.hasOwnProperty('name') && req.body.hasOwnProperty('uid') && req.body.hasOwnProperty('filePath') && req.body.hasOwnProperty('content')){
		...
		pyrofb.child('instances').child(appName).once('value', function(appSnap){
			...
			if(appData) {
				...
				saveToFileOnS3(bucketName, path, req.body.content).then(function(fileInfo){
...
/** Upload app file to s3 by pulling the information from Firebase
 * @function uploadFromRamList
 * @params {string} bucketName Name of bucket to upload to/from
 * @params {string} fileKey Name of bucket to upload to/from
 * @params {string} uid Unique Identification number of user
 */
function uploadFromRamList(argBucketName, argFileKey, argUid){
	console.log('uploadFromRamList called', arguments);
	var deferred = Q.defer()
	// File reference in userRam list
	var fileRef = pyrofb.child('userRam').child(argUid).child(argBucketName).child(argFileKey);
	// Load File
	//[TODO] Check to make sure that edit is by the author/a contributor
	fileRef.once('value', function(fileSnap){
		if(fileSnap.val()) {
			//file exists in file ref
	...
	createFirebaseInstance(argUid, firebaseObj.dbName).then(function(returnedFbUrl){
	  console.log('[generatePyroApp] create firebase successfully returned:', returnedFbUrl);
		...
		createAndConfigS3Bucket(firebaseObj.dbName).then(function() {
  		...
  		console.error('[generatePyroApp] Error Creating S3 bucket:', err);
			deferred.reject(err);
  	});
	}, function(err){
		console.error('[generatePyroApp] Error creating firebase instance, err:', err);
		//[TODO] Handle Firebase instance already exisiting
		deferred.reject(err);
	});
	return deferred.promise;
}
...
function uploadToBucket(argBucketName, argLocalDir, argAppName){
	...
	fs.copy(argLocalDir, newAppDir , function(err){
		...
		uploader.on('error', function(err) {
	  	...
	  	//If bucket doesn't exist try without pyro- infront
	  	if(err.code == "NOSUCHBUCKET"){
	  		var oldBucketName = upParams.s3Params.Bucket;
	  		upParams.s3Params.Bucket = argBucketName.replace("pyro-", "");
	  		if(oldBucketName != upParams.s3Params.Bucket){
	  			var secondTryUploader = client.uploadDir(upParams);
	  			secondTryUploader.on('error', function(err) {
				  	console.error("[uploadToBucket] unable to sync:", err);
				  	//If bucket doesn't exist try without pyro- infront
						deferred.reject({status:500, error:err});
					});
	  			secondTryUploader.on('end', function() {
